...
#import "QN_uploadFileWithParams.h"

@interface QN_uploadFileWithParams ()

@property (nonatomic, strong) UIImage * pickImage;

@end

@implementation QN_uploadFileWithParams
...
-(void)getTokenFromQN
{
    [HTTPRequestPost hTTPRequest_GetpostBody:nil andUrl:@"~aaron/qiniu-api-server/php-v6/api/simple_upload/use_return_body_upload_token.php" andSucceed:^(NSURLSessionDataTask *task, id responseObject) {
...
-(void)uploadImageToQNFilePath:(NSString *)filePath
{
    ...
    NSDictionary * parma = @{@"x:exParam1":self.fillParam1.text,
                             @"x:exParam2":self.fillParam2.text};
    
    QNUploadManager *upManager = [[QNUploadManager alloc] init];
    QNUploadOption * uploadOption = [[QNUploadOption alloc] initWithMime:nil progressHandler:^(NSString *key, float percent) {
        self.percentFloat = percent;
    } params:parma checkCrc:NO cancellationSignal:nil];
